 ...
 *  each property.
 *
 *  A compound may consist of a single or multiple components (E.g., BeF2,
 * LiF-NaF-KF)
 *
 * Units:
...
public:
  ...
  struct View {
    ...
    double speedOfSound_unc() const;
    std::pair<double, double> speedOfSound_rng() const;
    std::string speedOfSound_ref() const;
  ...
  getSaltComps(Vec_Name names) const = 0;

  // specific heat
  virtual double cp(Id id, Id data_id, double temperature,
                    double pressure = 101.325) const = 0;
  virtual double cp_h(Id id, Id data_id, double enthalpy,
                      double pressure = 101.325) const = 0;
  ...
  // viscosity
  virtual double mu(Id id, Id data_id, double temperature,
                    double pressure = 101.325) const = 0;
  virtual double mu_h(Id id, Id data_id, double enthalpy,
                      double pressure = 101.325) const = 0;
  ...
  // conductivity
  virtual double k(Id id, Id data_id, double temperature,
                   double pressure = 101.325) const = 0;
  virtual double k_h(Id id, Id data_id, double enthalpy,
                     double pressure = 101.325) const = 0;
  ...
  // density
  virtual double rho(Id id, Id data_id, double temperature,
                     double pressure = 101.325) const = 0;
  virtual double rho_h(Id id, Id data_id, double enthalpy,
                       double pressure = 101.325) const = 0;
  ...
  // density
  virtual double surfaceTension(Id id, Id data_id, double temperature,
                                double pressure = 101.325) const = 0;
  virtual double surfaceTension_h(Id id, Id data_id, double enthalpy,
                                  double pressure = 101.325) const = 0;
  virtual bool valid_surfaceTension(Id id, Id data_id) const = 0;
  virtual double surfaceTension_unc(Id id, Id data_id) const = 0;
  virtual std::pair<double, double> surfaceTension_rng(Id id,
                                                       Id data_id) const = 0;
  virtual std::string surfaceTension_ref(Id id, Id data_id) const = 0;

  // speed of sound
  virtual double speedOfSound(Id id, Id data_id, double temperature) const = 0;
  virtual double speedOfSound_h(Id id, Id data_id, double enthalpy,
                                double pressure = 101.325) const = 0;
  virtual bool valid_speedOfSound(Id id, Id data_id) const = 0;
  virtual double speedOfSound_unc(Id id, Id data_id) const = 0;
  virtual std::pair<double, double> speedOfSound_rng(Id id,
                                                     Id data_id) const = 0;
  virtual std::string speedOfSound_ref(Id id, Id data_id) const = 0;
  ...
  // number of ions
  virtual double n_ions(Id id, Id data_id) const = 0;

  // anion/cation ratio
  virtual int complexation(Id id, Id data_id) const = 0;
  ...
  virtual View setView(const Vec_Name &names,
                       const Vec_Mole &mole_percents) = 0;
